...
@interface SCNavTabBar () <SCPopViewDelegate>
{
    ...
    UIView          *_superLine;             // lineView's backgroundView
...
- (void)showLineWithButtonWidth:(CGFloat)width
{
    ...
    _superLine = [[UIView alloc]initWithFrame:CGRectMake(2.0f, NAV_TAB_BAR_HEIGHT - 3.0f, SCREEN_WIDTH , 3.0f)];
    _superLine.backgroundColor = UIColorWithRGBA(240.0f, 239.0f, 245.0f, 1.0f);
    [_navgationTabBar addSubview:_superLine];
    _line = [[UIView alloc] initWithFrame:CGRectMake(2.0f, NAV_TAB_BAR_HEIGHT - 3.0f, width - 4.0f, 3.0f)];
    _line.backgroundColor = UIColorWithRGBA(248.0f, 97.0f, 111.0f, 0.7f);
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        [button setTitleColor:HJRGBA(100, 100, 100, 1.0) forState:UIControlStateNormal];
        [button setTitleColor:HJRGBA(248, 97, 111, 1.0) forState:UIControlStateSelected];
...
- (void)itemPressed:(UIButton *)button
{
    for (UIButton *btn in _items) {
        if ([btn isEqual:button]) btn.selected = YES;
...
//    }
    for (int i = 0; i < titles.count; i ++) {
        [widths addObject:@(SCREEN_WIDTH / titles.count)];
    }
    return widths;
}
